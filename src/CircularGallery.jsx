...
class Title {
  ...
  onResize() {
    if (!this.mesh) return;
    const canvas = this.mesh.program.uniforms.tMap.value.image;
    if (!canvas) return;
    ...
    const absoluteTextHeight = parentScaleY * 0.11; 
    const absoluteTextWidth = absoluteTextHeight * aspect;
    
    this.mesh.scale.set(absoluteTextWidth / parentScaleX, absoluteTextHeight / parentScaleY, 1);
    this.mesh.position.y = -0.5 - (absoluteTextHeight / parentScaleY) * 0.5 - (0.06 / parentScaleY);
  }
}
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      app = new AppEngine(containerRef.current, {
